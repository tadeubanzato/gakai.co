...
const accountDetailsWithN8n=accountDetails;accountDetails=async account=>{await accountDetailsWithN8n(account);const buttons=[...root.querySelectorAll('[data-service]')],n8n=root.querySelector('[data-service="n8n"]'),id=encodeURIComponent(account.id);const automations=async()=>api(`/api/app/accounts/${id}/automations`).catch(()=>({subscriptions:[]}));const keyState=async()=>api(`/api/app/accounts/${id}/integration-keys/n8n`).catch(()=>({token:null}));const setSelected=button=>buttons.forEach(item=>item.classList.toggle('on',item===button));const renderOther=button=>{setSelected(button);root.querySelector('#service-detail').innerHTML=`<h3>${esc(button.textContent.trim())}</h3><p>Connect this service when its integration is available.</p>`};const renderN8n=async()=>{setSelected(n8n);const panel=root.querySelector('#service-detail');let status;try{status=await api(`/api/app/accounts/${id}/n8n/connect`);}catch{status={connected:false};}if(!status.connected){n8n.classList.remove('has-integration');panel.innerHTML='<h3>Connect n8n</h3><p>Gakai will create a ready-to-use WhatsApp workflow in your n8n automatically.</p><form id="n8n-connect"><label>n8n URL<input id="n8n-url" type="url" required placeholder="https://yourname.app.n8n.cloud"></label><label>n8n API key<input id="n8n-api-key" type="password" required placeholder="Paste your n8n API key"><small id="n8n-api-link">Get your key from <a id="n8n-settings-link" href="#" target="_blank" rel="noopener">Settings → API</a> in your n8n instance</small></label><button class="primary" id="n8n-connect-btn">Connect to n8n</button></form><div id="n8n-status"></div>';const urlInput=panel.querySelector('#n8n-url'),settingsLink=panel.querySelector('#n8n-settings-link');urlInput.addEventListener('input',()=>{try{const u=new URL(urlInput.value.trim());settingsLink.href=u.origin+'/settings/api';}catch{settingsLink.href='#';}});panel.querySelector('#n8n-connect').onsubmit=async e=>{e.preventDefault();const btn=panel.querySelector('#n8n-connect-btn'),statusDiv=panel.querySelector('#n8n-status');btn.disabled=true;statusDiv.textContent='Connecting…';try{const result=await api(`/api/app/accounts/${id}/n8n/connect`,{method:'POST',body:JSON.stringify({n8nUrl:urlInput.value.trim(),n8nApiKey:panel.querySelector('#n8n-api-key').value.trim()})});await renderN8n();if(result.activationWarning)notice('Workflow created but activation failed — open it in n8n and activate manually.');}catch(err){statusDiv.textContent=err.message||'Connection failed';btn.disabled=false;}};}else{n8n.classList.add('has-integration');const timeAgo=iso=>{if(!iso)return'never';const diff=Date.now()-new Date(iso).getTime();if(diff<60000)return'just now';if(diff<3600000)return Math.floor(diff/60000)+'m ago';if(diff<86400000)return Math.floor(diff/3600000)+'h ago';return Math.floor(diff/86400000)+'d ago';};const deliveryStatus=status.lastDelivery?`${status.lastDelivery.ok?'OK':'Failed'} — ${timeAgo(status.lastDelivery.at)}`:'No deliveries yet';panel.innerHTML=`<h3>n8n connected</h3><p>✓ Gakai is sending WhatsApp messages to your n8n workflow automatically.</p><div class="n8n-connected-card"><b></b><a href="${esc(status.workflowUrl||'#')}" target="_blank" rel="noopener" class="secondary">Open workflow in n8n ↗</a></div><div class="n8n-delivery-status">Last delivery: ${esc(deliveryStatus)}</div><div class="n8n-test-section"><h4>Send a test message</h4><input id="n8n-test-phone" inputmode="tel" placeholder="Phone number (e.g. 15551234567)"><input id="n8n-test-message" placeholder="Test message text"><button type="button" class="secondary" id="n8n-test-btn">Send test</button></div><hr><button type="button" class="danger" id="n8n-disconnect">Disconnect n8n</button>`;panel.querySelector('.n8n-connected-card b').textContent=status.workflowName||'n8n workflow';panel.querySelector('#n8n-test-btn').onclick=async()=>{const phone=panel.querySelector('#n8n-test-phone').value.trim(),text=panel.querySelector('#n8n-test-message').value.trim();try{const subs=await api(`/api/app/accounts/${id}/automations`).catch(()=>({subscriptions:[]}));const sub=subs.subscriptions?.find(s=>s.name==='n8n auto-connect')||subs.subscriptions?.[0];if(!sub)return notice('No automation subscription found');await api(`/api/app/accounts/${id}/automations/${encodeURIComponent(sub.id)}/test`,{method:'POST',body:JSON.stringify({phone,text:text||'This is a Gakai test event.',destination:'production'})});notice('Test event delivered to n8n');}catch(err){notice(err.message);}};panel.querySelector('#n8n-disconnect').onclick=async()=>{if(!confirm('Disconnect n8n? The workflow in n8n will remain but Gakai will stop sending events to it.'))return;try{await api(`/api/app/accounts/${id}/n8n/connect`,{method:'DELETE'});await renderN8n();}catch(err){notice(err.message);}};};};buttons.forEach(button=>button.onclick=button===n8n?renderN8n:()=>renderOther(button));const initial=await automations();n8n.classList.toggle('has-integration',Boolean(initial.subscriptions?.length));};

const LLM_SKILL_TEMPLATE=`## Identity
You are a helpful assistant for [your business or name].
Reply in: English
Tone: friendly and concise

## Scope — what to respond to
- Reply to direct messages only (not group chats)
- Only reply about: [your topics, e.g. support, bookings, product questions]
- Ignore: voice notes, media-only messages, spam

## Filters
- Only reply to these phone numbers or group IDs: (leave blank for all)
- Always active (no time restrictions)

## Reply rules
- Keep replies under 3 sentences unless the question requires more detail
- Never share: [sensitive info, e.g. pricing, personal data]
