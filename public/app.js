...
// AI enablement: a focused account settings page with a guided n8n webhook setup.
async function renderAutomationSettings(){const account=s.settingsAccount||s.account;if(!account){s.view="inbox";return render()}let subscriptions=[];try{subscriptions=(await api(`/api/app/accounts/${encodeURIComponent(account.id)}/automations`)).subscriptions||[]}catch(error){notice(error.message)}root.innerHTML=`<div class="settings-page"><header><div><button id="settings-back" class="secondary">‹ Inbox</button><h2>${esc(account.label)} settings</h2><small>Manage account access and automations.</small></div></header><main class="account-page settings-page-main"><section class="settings-section"><span class="eyebrow">AUTOMATIONS</span><h1>Connect n8n</h1><p>Send each new incoming WhatsApp message to an n8n workflow. In n8n, create a Webhook trigger, copy its <b>Production URL</b>, then paste it below.</p><form id="automation-create"><label>n8n production webhook URL<input id="automation-url" type="url" required placeholder="https://n8n.example.com/webhook/..." inputmode="url"></label><label>Automation name<input id="automation-name" maxlength="80" placeholder="Customer reply workflow"></label><button class="primary">Connect n8n</button></form><p class="automation-help">Gakai sends a <code>message.received</code> event only. Your workflow can filter a sender or group, then use the Gakai API key below to send a reply.</p></section><section class="settings-section"><h3>Connected automations</h3>${subscriptions.length?subscriptions.map(item=>`<article class="automation-card"><div><b>${esc(item.name)}</b><small>${esc(item.url)}</small><small>${item.enabled?"Enabled":"Paused"}${item.lastDelivery?` · Last delivery ${item.lastDelivery.ok?"succeeded":"failed"}`:" · Not tested yet"}</small></div><button class="secondary" data-automation-test="${esc(item.id)}">Test</button><button class="secondary" data-automation-toggle="${esc(item.id)}" data-enabled="${item.enabled}">${item.enabled?"Pause":"Enable"}</button><button class="danger compact" data-automation-delete="${esc(item.id)}">Remove</button></article>`).join(""):"<p>No n8n automations connected yet.</p>"}</section><section class="settings-section"><h3>Use Gakai from n8n</h3><p>Create an integration key with message read and send permissions. In n8n’s HTTP Request node, set <code>Authorization: Bearer YOUR_KEY</code> and call Gakai’s integration endpoints.</p><button class="secondary" id="settings-create-key">Create n8n API key</button><div id="settings-new-key"></div></section></main></div>`;root.querySelector("#settings-back").onclick=()=>{s.view="inbox";s.settingsAccount=null;render()};root.querySelector("#automation-create").onsubmit=async event=>{event.preventDefault();try{const data=await api(`/api/app/accounts/${encodeURIComponent(account.id)}/automations`,{method:"POST",body:JSON.stringify({url:root.querySelector("#automation-url").value,name:root.querySelector("#automation-name").value})});alert(`Connected. In n8n, require this header on the Webhook trigger:\nX-Gakai-Secret: ${data.secret}\n\nCopy it now; it is not shown again.`);await renderAutomationSettings()}catch(error){notice(error.message)}};root.querySelectorAll("[data-automation-test]").forEach(button=>button.onclick=async()=>{button.disabled=true;try{await api(`/api/app/accounts/${encodeURIComponent(account.id)}/automations/${encodeURIComponent(button.dataset.automationTest)}/test`,{method:"POST"});notice("Test event delivered") }catch(error){notice(error.message)}finally{await renderAutomationSettings()}});root.querySelectorAll("[data-automation-toggle]").forEach(button=>button.onclick=async()=>{await api(`/api/app/accounts/${encodeURIComponent(account.id)}/automations/${encodeURIComponent(button.dataset.automationToggle)}`,{method:"PATCH",body:JSON.stringify({enabled:button.dataset.enabled!=="true"})});renderAutomationSettings()});root.querySelectorAll("[data-automation-delete]").forEach(button=>button.onclick=async()=>{if(!confirm("Remove this n8n automation?"))return;await api(`/api/app/accounts/${encodeURIComponent(account.id)}/automations/${encodeURIComponent(button.dataset.automationDelete)}`,{method:"DELETE"});renderAutomationSettings()});root.querySelector("#settings-create-key").onclick=async()=>{try{const data=await api(`/api/app/accounts/${encodeURIComponent(account.id)}/integration-keys`,{method:"POST",body:JSON.stringify({name:"n8n",scopes:["messages:read","messages:send"]})});root.querySelector("#settings-new-key").innerHTML=`<p><b>Copy this n8n API key now.</b></p><code class="automation-secret">${esc(data.token)}</code>`}catch(error){notice(error.message)}}}
settings=async account=>{s.settingsAccount=account;s.view="settings";render()};
const renderBeforeAutomationSettings=render;render=()=>s.view==="settings"?renderAutomationSettings():renderBeforeAutomationSettings();

// Full account profile: account controls, per-account access, automations, and workspace login security.
async function renderAccountProfile(){const account=s.profileAccount||s.account;if(!account){s.view="inbox";return render()}const [keyData,automationData,profile]=await Promise.all([api(`/api/app/accounts/${encodeURIComponent(account.id)}/integration-keys`).catch(()=>({keys:[]})),api(`/api/app/accounts/${encodeURIComponent(account.id)}/automations`).catch(()=>({subscriptions:[]})),api("/api/app/auth/profile").catch(()=>({username:""}))]);const keys=keyData.keys||[],automations=automationData.subscriptions||[];root.innerHTML=`<div class="profile-page"><header class="profile-header"><button id="profile-back" class="secondary">‹ Inbox</button><div class="profile-heading">${avatar(account)}<span><span class="eyebrow">ACCOUNT PROFILE</span><h2>${esc(account.label)}</h2><small>${esc(account.phone||"WhatsApp account")} · ${esc(status(account.status))}</small></span></div></header><main class="profile-grid"><section class="profile-card profile-overview"><h3>Account details</h3><p>Keep this WhatsApp account recognizable across your workspace.</p><form id="profile-label"><label>Account name<input id="profile-label-input" maxlength="80" value="${esc(account.label)}"></label><button class="primary">Save account name</button></form><div class="profile-actions"><button id="profile-relink" class="secondary">Generate new QR code</button><button id="profile-delete" class="danger compact">Delete account</button></div></section><section class="profile-card"><div class="section-title"><div><h3>API keys</h3><p>Use a separate key for each app or automation. Keys are shown only once.</p></div><button id="profile-create-key" class="primary">Create API key</button></div><div id="profile-new-key"></div><div class="profile-list">${keys.length?keys.map(key=>`<article><div><b>${esc(key.name)}</b><small>${esc((key.scopes||[]).join(", "))} · ${key.lastUsedAt?"Used "+esc(key.lastUsedAt):"Never used"}</small></div><button class="secondary" data-profile-revoke="${esc(key.id)}">Revoke</button></article>`).join(""):"<p class="empty-copy">No API keys for this account.</p>"}</div></section><section class="profile-card"><div class="section-title"><div><h3>n8n automations</h3><p>New incoming messages can start a workflow in n8n.</p></div><button id="profile-automation" class="secondary">Manage n8n</button></div><div class="profile-list">${automations.length?automations.map(item=>`<article><div><b>${esc(item.name)}</b><small>${item.enabled?"Enabled":"Paused"} · ${item.lastDelivery?.ok?"Last delivery succeeded":"No successful delivery yet"}</small></div><button class="secondary" data-profile-automation="${esc(item.id)}">${item.enabled?"Pause":"Enable"}</button></article>`).join(""):"<p class="empty-copy">No n8n workflows connected.</p>"}</div></section><section class="profile-card profile-security"><span class="eyebrow">WORKSPACE SECURITY</span><h3>Sign-in details</h3><p>Change the administrator username or password. Confirm your current password to save changes.</p><form id="profile-security"><label>Username<input id="profile-username" minlength="3" maxlength="40" value="${esc(profile.username||"")}" autocomplete="username"></label><label>Current password<input id="profile-current-password" type="password" required autocomplete="current-password"></label><label>New password <small>Leave blank to keep your password.</small><input id="profile-new-password" type="password" minlength="10" autocomplete="new-password"></label><button class="primary">Save sign-in details</button></form></section></main></div>`;root.querySelector("#profile-back").onclick=()=>{history.pushState({},"","/");s.view="inbox";s.profileAccount=null;render()};root.querySelector("#profile-label").onsubmit=async event=>{event.preventDefault();try{await api(`/api/app/accounts/${encodeURIComponent(account.id)}/label`,{method:"PATCH",body:JSON.stringify({label:root.querySelector("#profile-label-input").value})});await refresh();notice("Account name saved")}catch(error){notice(error.message)}};root.querySelector("#profile-relink").onclick=async()=>{s.account=account;s.view="pairing";s.qr="";render();await restart(account.id);pollForQr()};root.querySelector("#profile-delete").onclick=async()=>{if(!confirm(`Delete ${account.label}?`))return;await api(`/api/app/accounts/${encodeURIComponent(account.id)}`,{method:"DELETE"});history.pushState({},"","/");s.view="inbox";await refresh()};root.querySelector("#profile-create-key").onclick=async()=>{try{const data=await api(`/api/app/accounts/${encodeURIComponent(account.id)}/integration-keys`,{method:"POST",body:JSON.stringify({name:"API key",scopes:["messages:read","messages:send"]})});root.querySelector("#profile-new-key").innerHTML=`<div class="profile-secret"><b>Copy this key now.</b><code>${esc(data.token)}</code></div>`;renderAccountProfile()}catch(error){notice(error.message)}};root.querySelectorAll("[data-profile-revoke]").forEach(button=>button.onclick=async()=>{if(!confirm("Revoke this API key?"))return;await api(`/api/app/accounts/${encodeURIComponent(account.id)}/integration-keys/${encodeURIComponent(button.dataset.profileRevoke)}`,{method:"DELETE"});renderAccountProfile()});root.querySelector("#profile-automation").onclick=()=>{s.settingsAccount=account;s.view="settings";render()};root.querySelectorAll("[data-profile-automation]").forEach(button=>button.onclick=async()=>{await api(`/api/app/accounts/${encodeURIComponent(account.id)}/automations/${encodeURIComponent(button.dataset.profileAutomation)}`,{method:"PATCH",body:JSON.stringify({enabled:button.textContent!=="Pause"})});renderAccountProfile()});root.querySelector("#profile-security").onsubmit=async event=>{event.preventDefault();try{const data=await api("/api/app/auth/profile",{method:"PATCH",body:JSON.stringify({username:root.querySelector("#profile-username").value,currentPassword:root.querySelector("#profile-current-password").value,newPassword:root.querySelector("#profile-new-password").value})});root.querySelector("#profile-current-password").value="";root.querySelector("#profile-new-password").value="";notice(`Sign-in details saved for ${data.username}`)}catch(error){notice(error.message)}}}
settings=async account=>{s.profileAccount=account;s.view="profile";history.pushState({},"",`/accounts/${encodeURIComponent(account.id)}/details`);render()};
const renderBeforeProfile=render;render=()=>s.view==="profile"?renderAccountProfile():renderBeforeProfile();
const refreshWithProfileRoute=refresh;refresh=async()=>{await refreshWithProfileRoute();const match=location.pathname.match(/^\/accounts\/([^/]+)\/details$/);if(match&&s.view!=="profile"){const account=s.accounts.find(item=>item.id===decodeURIComponent(match[1]));if(account){s.profileAccount=account;s.view="profile";render()}}};
window.addEventListener("popstate",()=>{const match=location.pathname.match(/^\/accounts\/([^/]+)\/details$/);const account=match&&s.accounts.find(item=>item.id===decodeURIComponent(match[1]));s.profileAccount=account||null;s.view=account?"profile":"inbox";render()});
