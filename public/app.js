...
function sidebar(){return `<aside class="sidebar"><div class="logo"><b>W</b> Gakai</div><div class="account-switch">${s.accounts.map(a=>`<button data-account="${esc(a.id)}" class="account ${a.id===current()?'selected':''}"><i class="${a.status==='WORKING'?'good':''}"></i><span>${esc(a.label)}<small>${status(a.status)}</small></span></button>`).join('')}</div></aside>`}
...
sidebar=()=>`<aside class="sidebar ${sidebarCollapsed?'collapsed':''}"><div class="logo" title="${sidebarCollapsed?'Expand sidebar':'Gakai'}"><b>W</b><span>Gakai</span></div><div class="account-switch">${s.accounts.map(account=>`<button data-account="${esc(account.id)}" title="${esc(account.label)}" class="account ${account.id===current()?'selected':''}"><i class="${account.status==='WORKING'?'good':''}"></i>${avatar(account)}<span><b>${esc(account.label)}</b><small>${status(account.status)}</small></span></button>`).join('')}</div></aside>`;
