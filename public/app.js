...
const s={accounts:[],account:null,chats:[],chat:null,messages:[],view:'inbox',adding:false,qr:'',search:''};
...
// Clean account details page: one renderer, no modal or layered overrides.
async function accountDetails(account){const profile=await api("/api/app/auth/profile").catch(()=>({username:""}));s.view="details";root.innerHTML=`<div class="details"><header class="details-head"><div class="details-identity"><img class="details-avatar" src="${esc(account.picture||"")}" alt="" onerror="this.style.display='none'"><div><span class="eyebrow">ACCOUNT DETAILS</span><h2>${esc(account.label)}</h2><small>Phone: ${esc(account.phone||"Not connected")}</small></div></div><button id="details-back" class="secondary">‹ Inbox</button></header><main class="details-main"><div class="details-top"><section class="details-card"><h3>Account name</h3><p>Name this WhatsApp account for your workspace.</p><form id="details-name"><input id="details-name-value" value="${esc(account.label)}" maxlength="80"><button class="primary">Save</button></form></section><section class="details-card"><h3>Integrations</h3><p>Connect automation, AI, or custom services with separate account-scoped API keys.</p><small>Keys are shown only once and can be revoked later.</small></section></div><section class="details-card security"><h3>Administrator profile</h3><p>Update your sign-in username or password.</p><form id="details-security"><input id="details-user" value="${esc(profile.username||"")}" placeholder="Username"><input id="details-current" type="password" placeholder="Current password" required><input id="details-new" type="password" placeholder="New password (optional)"><button class="primary">Save sign-in details</button></form></section><section class="details-card services"><div class="services-list"><h3>Services</h3><button data-service="n8n">n8n <small>Automation</small><i>✓</i></button><button data-service="claude">Claude <small>AI workflows</small><i>✓</i></button><button data-service="codex">Codex <small>Agentic development</small><i>✓</i></button><button data-service="api">Standalone API <small>Custom services</small><i>✓</i></button></div><div id="service-detail" class="service-detail"><h3>Select services</h3><p>Choose one or more services from the list to create their dedicated API keys.</p></div></section></main></div>`;root.querySelector("#details-back").onclick=()=>{history.pushState({},"","/");s.view="inbox";render()};root.querySelector("#details-name").onsubmit=async e=>{e.preventDefault();await api(`/api/app/accounts/${encodeURIComponent(account.id)}/label`,{method:"PATCH",body:JSON.stringify({label:root.querySelector("#details-name-value").value})});notice("Account name saved")};root.querySelector("#details-security").onsubmit=async e=>{e.preventDefault();try{await api("/api/app/auth/profile",{method:"PATCH",body:JSON.stringify({username:root.querySelector("#details-user").value,currentPassword:root.querySelector("#details-current").value,newPassword:root.querySelector("#details-new").value})});notice("Sign-in details saved")}catch(error){notice(error.message)}};const selected=new Set(),labels={n8n:"n8n",claude:"Claude",codex:"Codex",api:"Standalone API"},detail=root.querySelector("#service-detail");root.querySelectorAll("[data-service]").forEach(button=>button.onclick=()=>{const service=button.dataset.service;if(selected.has(service)){selected.delete(service);button.classList.remove("on")}else{selected.add(service);button.classList.add("on")}detail.innerHTML="";if(!selected.size){detail.innerHTML="<h3>Select services</h3><p>Choose one or more services from the list to create their dedicated API keys.</p>";return}selected.forEach(service=>{const row=document.createElement("div"),title=document.createElement("b"),action=document.createElement("button"),out=document.createElement("div");title.textContent=labels[service];action.className="primary";action.textContent=`Create ${labels[service]} API key`;action.onclick=async()=>{const data=await api(`/api/app/accounts/${encodeURIComponent(account.id)}/integration-keys`,{method:"POST",body:JSON.stringify({name:`${labels[service]} integration`,scopes:["messages:read","messages:send"]})});out.innerHTML=`<code>${esc(data.token)}</code>`};row.append(title,action,out);detail.append(row)})})}
settings=async account=>{history.pushState({},"",`/accounts/${encodeURIComponent(account.id)}/details`);accountDetails(account)};
const detailsRefresh=refresh;refresh=async()=>{await detailsRefresh();const match=location.pathname.match(/^\/accounts\/([^/]+)\/details$/);const account=match&&s.accounts.find(item=>item.id===decodeURIComponent(match[1]));if(account&&s.view!=="details")accountDetails(account)};
const accountDetailsWithN8n=accountDetails;accountDetails=async account=>{await accountDetailsWithN8n(account);const buttons=[...root.querySelectorAll('[data-service]')],n8n=root.querySelector('[data-service="n8n"]'),id=encodeURIComponent(account.id);const automations=async()=>api(`/api/app/accounts/${id}/automations`).catch(()=>({subscriptions:[]}));const keyState=async()=>api(`/api/app/accounts/${id}/integration-keys/n8n`).catch(()=>({token:null}));const setSelected=button=>buttons.forEach(item=>item.classList.toggle('on',item===button));const renderOther=button=>{setSelected(button);root.querySelector('#service-detail').innerHTML=`<h3>${esc(button.textContent.trim())}</h3><p>Connect this service when its integration is available.</p>`};const renderN8n=async()=>{setSelected(n8n);const panel=root.querySelector('#service-detail'),data=await automations(),saved=data.subscriptions?.[0],key=await keyState();n8n.classList.toggle('has-integration',Boolean(saved));panel.innerHTML='<h3>n8n automation</h3><p>Save your active n8n production webhook. Gakai sends incoming WhatsApp messages to this workflow.</p><form id="n8n-connect"><label>Production webhook URL<input id="n8n-production-url" type="url" required placeholder="https://n8n.example.com/webhook/..." aria-label="n8n production webhook URL"></label><label>Test webhook URL<input id="n8n-test-url" type="url" readonly disabled placeholder="Auto-generated from production URL" aria-label="n8n test webhook URL"></label><label>Test phone number <small>(optional)</small><input id="n8n-test-phone" inputmode="tel" placeholder="5511979619999" aria-label="Test phone number"></label><label>Workflow name <small>(optional)</small><input id="n8n-name" placeholder="Customer replies" aria-label="n8n workflow name"></label><button class="primary">Save integration</button></form><small>n8n Webhook settings: Method = <b>POST</b>; Authentication = <b>Header Auth</b>; Name = <code>X-Gakai-Secret</code>; Value = the webhook secret shown below; Response = <b>Immediately</b>.</small><div id="n8n-output"></div><div id="n8n-list"></div>';const productionUrl=panel.querySelector('#n8n-production-url'),testUrl=panel.querySelector('#n8n-test-url'),testPhone=panel.querySelector('#n8n-test-phone'),name=panel.querySelector('#n8n-name'),output=panel.querySelector('#n8n-output'),list=panel.querySelector('#n8n-list');const syncTestUrl=()=>{testUrl.value=productionUrl.value.replace('/webhook/','/webhook-test/')};productionUrl.addEventListener('input',syncTestUrl);const icon=(kind)=>kind==='copy'?'<svg viewBox="0 0 24 24" aria-hidden="true"><rect x="8" y="8" width="11" height="11" rx="2" fill="none" stroke="currentColor" stroke-width="2"/><path d="M5 16V5a2 2 0 0 1 2-2h9" fill="none" stroke="currentColor" stroke-width="2"/></svg>':'<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M20 11a8 8 0 0 0-14.8-4L3 10m0-5v5h5M4 13a8 8 0 0 0 14.8 4L21 14m0 5v-5h-5" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>';const copy=async text=>{try{if(navigator.clipboard)await navigator.clipboard.writeText(text);else throw Error()}catch{const area=document.createElement('textarea');area.value=text;area.style.position='fixed';area.style.opacity='0';document.body.append(area);area.select();document.execCommand('copy');area.remove()}notice('API key copied to clipboard')};const showKey=token=>{if(!token){output.replaceChildren();return}output.innerHTML='<div class="profile-secret"><b>Gakai API key for n8n HTTP Request nodes:</b><div class="n8n-key-row"><code></code><button type="button" class="icon-button" id="n8n-copy" aria-label="Copy API key" title="Copy API key">'+icon('copy')+'</button><button type="button" class="icon-button" id="n8n-refresh" aria-label="Refresh API key" title="Refresh API key">'+icon('refresh')+'</button></div></div>';output.querySelector('code').textContent=token;output.querySelector('#n8n-copy').onclick=()=>copy(token);output.querySelector('#n8n-refresh').onclick=async()=>{if(!confirm('Refresh this API key? Existing n8n requests using it will stop working.'))return;showKey((await api(`/api/app/accounts/${id}/integration-keys/n8n`,{method:'POST'})).token)}};showKey(key.token);if(saved){productionUrl.value=saved.productionUrl||saved.url;testUrl.value=saved.testUrl||"";testPhone.value=saved.testPhone||"";name.value=saved.name;const row=document.createElement('div');row.className='n8n-saved-row';const badge=document.createElement('span');badge.className='n8n-saved-badge';badge.textContent='✓';const label=document.createElement('small');label.textContent=`Saved: ${saved.name} · ${saved.enabled?'Enabled':'Paused'}`;const test=document.createElement('button');test.type='button';test.className='secondary';test.textContent='Test production webhook';test.onclick=async()=>{const phone=window.prompt("Test phone number (digits only)",testPhone.value||"");if(phone===null)return;const text=window.prompt("Test message", "Test");if(text===null)return;try{await api(`/api/app/accounts/${id}/automations/${encodeURIComponent(saved.id)}/test`,{method:"POST",body:JSON.stringify({phone,text,destination:"production"})});notice("Production test event delivered to n8n")}catch(error){notice(error.message)}};const testUrlButton=document.createElement("button");testUrlButton.type="button";testUrlButton.className="secondary";testUrlButton.textContent="Test test webhook";testUrlButton.onclick=async()=>{try{await api(`/api/app/accounts/${id}/automations/${encodeURIComponent(saved.id)}/test`,{method:"POST",body:JSON.stringify({phone:testPhone.value,destination:"test"})});notice("Test URL event delivered to n8n")}catch(error){notice(error.message)}};row.append(badge,label,test,testUrlButton);list.append(row)}panel.querySelector('#n8n-connect').onsubmit=async event=>{event.preventDefault();try{const keyData=await api(`/api/app/accounts/${id}/integration-keys/n8n`);const token=keyData.token||(await api(`/api/app/accounts/${id}/integration-keys/n8n`,{method:'POST'})).token;await api(`/api/app/accounts/${id}/automations`,{method:'POST',body:JSON.stringify({productionUrl:productionUrl.value,testUrl:testUrl.value,testPhone:testPhone.value,name:name.value,secret:token})});showKey(token);n8n.classList.add('has-integration');await renderN8n();notice('n8n integration saved')}catch(error){notice(error.message)}}};buttons.forEach(button=>button.onclick=button===n8n?renderN8n:()=>renderOther(button));const initial=await automations();n8n.classList.toggle('has-integration',Boolean(initial.subscriptions?.length));};
const accountDetailsWithWebhookSecret=accountDetails;accountDetails=async account=>{await accountDetailsWithWebhookSecret(account);const button=root.querySelector('[data-service="n8n"]');if(!button)return;const prior=button.onclick;button.onclick=async()=>{await prior();const data=await api(`/api/app/accounts/${encodeURIComponent(account.id)}/automations`).catch(()=>({subscriptions:[]})),sub=data.subscriptions?.[0];if(!sub?.secret)return;const output=root.querySelector('#n8n-output');if(!output||output.querySelector('.n8n-webhook-secret'))return;const block=document.createElement('div');block.className='profile-secret n8n-webhook-secret';block.innerHTML='<b>n8n Webhook Header Auth → Value:</b><div class="n8n-key-row"><code></code><button type="button" class="icon-button n8n-secret-copy" aria-label="Copy webhook secret" title="Copy webhook secret"><svg viewBox="0 0 24 24" aria-hidden="true"><rect x="8" y="8" width="11" height="11" rx="2" fill="none" stroke="currentColor" stroke-width="2"/><path d="M5 16V5a2 2 0 0 1 2-2h9" fill="none" stroke="currentColor" stroke-width="2"/></svg></button></div>';block.querySelector('code').textContent=sub.secret;block.querySelector('button').onclick=async()=>{const text=sub.secret;try{if(navigator.clipboard)await navigator.clipboard.writeText(text);else throw Error()}catch{const area=document.createElement('textarea');area.value=text;area.style.position='fixed';area.style.opacity='0';document.body.append(area);area.select();document.execCommand('copy');area.remove()}notice('Webhook secret copied')};output.prepend(block)}};
