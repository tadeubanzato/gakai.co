...
// n8n has a complete setup path: event webhook in, scoped Gakai API key out.
const accountDetailsWithN8n=accountDetails;
accountDetails=async account=>{await accountDetailsWithN8n(account);const n8n=root.querySelector("[data-service=\"n8n\"]");if(!n8n)return;n8n.onclick=async()=>{root.querySelectorAll("[data-service]").forEach(button=>button.classList.toggle("on",button===n8n));const detail=root.querySelector("#service-detail");const existing=await api(`/api/app/accounts/${encodeURIComponent(account.id)}/automations`).catch(()=>({subscriptions:[]}));detail.innerHTML=`<h3>n8n automation</h3><p>Paste an active n8n Webhook Trigger production URL. Gakai sends each incoming message there, and n8n uses its Gakai API key to read or reply.</p><form id="n8n-connect"><input id="n8n-url" type="url" required placeholder="https://n8n.example.com/webhook/..."><input id="n8n-name" placeholder="Customer reply workflow"><button class="primary">Connect n8n</button></form><button id="n8n-key" class="secondary">Create n8n API key</button><div id="n8n-output"></div><div id="n8n-list"></div>`;const output=detail.querySelector("#n8n-output"),list=detail.querySelector("#n8n-list");const renderSubscriptions=data=>{list.innerHTML="";(data.subscriptions||[]).forEach(item=>{const row=document.createElement("div"),label=document.createElement("small"),test=document.createElement("button");label.textContent=`${item.name} · ${item.enabled?"Enabled":"Paused"}`;test.className="secondary";test.textContent="Send test";test.onclick=async()=>{try{await api(`/api/app/accounts/${encodeURIComponent(account.id)}/automations/${encodeURIComponent(item.id)}/test`,{method:"POST"});notice("Test event delivered to n8n")}catch(error){notice(error.message)}};row.append(label,test);list.append(row)})};renderSubscriptions(existing);detail.querySelector("#n8n-connect").onsubmit=async event=>{event.preventDefault();try{const data=await api(`/api/app/accounts/${encodeURIComponent(account.id)}/automations`,{method:"POST",body:JSON.stringify({url:detail.querySelector("#n8n-url").value,name:detail.querySelector("#n8n-name").value})});output.innerHTML=`<div class="profile-secret"><b>In n8n, require this header:</b><code>X-Gakai-Secret: ${esc(data.secret)}</code></div>`;renderSubscriptions(await api(`/api/app/accounts/${encodeURIComponent(account.id)}/automations`));notice("n8n connected") }catch(error){notice(error.message)}};detail.querySelector("#n8n-key").onclick=async()=>{try{const data=await api(`/api/app/accounts/${encodeURIComponent(account.id)}/integration-keys`,{method:"POST",body:JSON.stringify({name:"n8n integration",scopes:["messages:read","messages:send"]})});output.innerHTML=`<div class="profile-secret"><b>Use this in n8n HTTP Request nodes:</b><code>${esc(data.token)}</code></div>`}catch(error){notice(error.message)}}}};
